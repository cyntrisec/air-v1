...
| Required | Yes |
| Semantics | SHA-256 binding for the model artifact set used for the inference. |
| Verification | MHASH check: verifier MUST compare against a known-good hash when model identity matters. This is application-layer model identity evidence, not by itself proof of hardware-attested loading or execution. |
...
| CBOR type | tstr |
| Required | Yes |
| Semantics | Generic trust-state classification of the workload. AIR v1 defines a closed set of interoperable values: `production` and `evaluation`. |
| Verification | Verifier MUST reject unknown values. Receipts marked `evaluation` MUST NOT satisfy production trust policy. Vendor-specific submodes are out of scope for AIR v1 and SHOULD be mapped to the generic value space. |
