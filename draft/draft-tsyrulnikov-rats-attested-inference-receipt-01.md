...
abbrev: "AIR v1"
docname: draft-tsyrulnikov-rats-attested-inference-receipt-01
category: info
ipr: trust200902
submissiontype: IETF
area: Security
  ...
  I-D.messous-eat-ai:
    ...
    author:
      ...
      -
        ins: P. C. Liu
    date: 2026-02-23
    target: https://datatracker.ietf.org/doc/draft-messous-eat-ai/
  SCITT:
...
mechanisms for additional claim or platform profiles are out of scope.
...
: An entity that validates an AIR receipt's signature, claim values,
  ...
  attestation evidence and signing-key binding. In RATS {{RFC9334}}
  terminology, this maps to the Verifier role.
...
Endorser:
: A party that provides endorsements or reference values used to
...
The signing algorithm is Ed25519 with strict verification semantics
per {{RFC8032}} Section 5.1.7. Verifiers MUST reject non-canonical S
...
deterministic encoding per {{RFC8949}} Section 4.2.1, with keys sorted
by the bytewise lexicographic order of their deterministic encodings.
...
AIR v1 encodes `iat` as an unsigned integer only, consistent with the
time-encoding restriction in {{RFC9711}} Section 4.3.1. AIR v1 does
not use `exp` or `nbf`.
...
Verifiers that require TEE assurance MUST independently obtain and
...
The TDX registers are mapped to `pcr0`/`pcr1`/`pcr2` portability slots
...
imply that Nitro PCRs and TDX MRTD/RTMRs are semantically identical.
...
12. **Deterministic encoding**: Required. Map keys sorted per
    {{RFC8949}} Section 4.2.1 by the bytewise lexicographic order of
...
AIR can be incorporated into a RATS {{RFC9334}} architecture as
follows:

- The confidential workload is the **Attester**.
- A deployment-specific **Verifier** can evaluate AIR receipts
  together with platform attestation evidence and applicable
  endorsements or reference values.
- The end user, auditor, or compliance officer is the **Relying
  Party** (they consume verification results).
- The TEE platform vendor or attestation service providing
  endorsements or reference values acts as the **Endorser**.

AIR v1 is a workload-emitted signed application-layer artifact, not a
...
as part of the evidence it considered.
...
TEE attestation. A verifier MUST NOT infer TEE provenance from the AIR
...
end-to-end TEE assurance MUST verify, using a platform-specific
...
Nitro attestation documents carry a `public_key` field, while TDX
deployments commonly bind via `REPORTDATA`). Implementations that
require end-to-end TEE assurance SHOULD:

1.  Generate the Ed25519 key inside the TEE at startup.
2.  Include the public key in the platform attestation document
    (e.g., Nitro `public_key` field, TDX `REPORTDATA`).
...
Maturity:
: Reference implementation with end-to-end validation on three
  confidential-computing platforms.

Coverage:
: Validated against the 10-vector AIR corpus (2 valid, 8 invalid) and
  exercised in cross-cloud E2E runs.
...
The payload (CWT claims map) includes the 16 required claims:
